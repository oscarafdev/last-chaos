...
{
	LOG_INFO("LOGIN CDescriptor::GetLogin / Start...");
	LOG_INFO("LOGIN CDescriptor::GetLogin / msg->m_mtype [%d]", msg->m_mtype); // ### MSG_LOGIN = 3
	...
	try
	{
		...
		{
			LOG_INFO("LOGIN CDescriptor::GetLogin / msg->m_mtype != MSG_LOGIN");
		...
		int len_id = strlen(packet->id);
		int len_pw = strlen(packet->pw);

		//LOG_INFO("LOGIN CDescriptor::GetLogin / id_len:    [%d]", len_id);
		LOG_INFO("LOGIN CDescriptor::GetLogin / id:        [%s]", (const char *)packet->id);
		//LOG_INFO("LOGIN CDescriptor::GetLogin / pw_len:    [%d]", len_pw);
		//LOG_INFO("LOGIN CDescriptor::GetLogin / pw:        [%s]", (const char *)packet->pw);
		//LOG_INFO("LOGIN CDescriptor::GetLogin / cl nation: [%d] srv nation: [%d]"  , packet->nation, gserver.m_national);
...
*/
		...
		if(!CCheckPassword::checkPass(pass, dbpass))
			throw MSG_FAIL_LOGINSERV_WRONG_PASSWORD;
#elif defined(RUS_BCRYPT) && !defined(PASSWORD_ENCRYPT_SHA256)
		CLCString buf(1024);
		buf.Format("SELECT user_code, user_id, passwd FROM bg_user WHERE user_id='%s'",(const char*)m_idname);
		CDBCmd dbAuth;
		dbAuth.Init(&gserver.m_dbAuth);
		...
		//LOG_INFO("LOGIN CDescriptor::GetLogin / lowerid:   [%s]", (const char*)lowerid);
		//LOG_INFO("LOGIN CDescriptor::GetLogin / salt:      [%s]", (const char*)gserver.m_salt);
		//LOG_INFO("LOGIN CDescriptor::GetLogin / passwd:    [%s]", (const char*)m_passwd);
		...
		//LOG_INFO("LOGIN CDescriptor::GetLogin / idname:    [%s]", (const char*)m_idname);
		//LOG_INFO("LOGIN CDescriptor::GetLogin / hash:      [%s]", (const char*)gserver.m_hash);
		...
		sql.Format("SELECT user_code, user_id, passwd FROM bg_user WHERE user_id='%s'",(const char*)m_idname); // ###
#else // #ifdef LC_TLD
#ifdef EMPAS_LOGIN
		...
		if (!dbAuth.Open())
		{
			//LOG_INFO("LOGIN CDescriptor::GetLogins Faled dbAuth.Open()");
			throw MSG_FAIL_LOGINSERV_WRONG_PASSWORD;
		}

		if (!dbAuth.MoveFirst())
		{
			//LOG_INFO("LOGIN CDescriptor::GetLogins Faled dbAuth.MoveFirst()");
			throw MSG_FAIL_LOGINSERV_WRONG_PASSWORD;
		}
#endif
		int usercode;

#if defined (PASSWORD_ENCRYPT_SHA256)
		// ###
		CLCString _dbpassword(512);
		dbAuth.GetRec("passwd", _dbpassword);
		//LOG_INFO("LOGIN CDescriptor::GetLogin / dbpassword:[%s]", (const char*)dbpassword);

		std::string _dbpass((const char*)_dbpassword);
		std::string _pass((const char*)gserver.m_hash);

		if(_pass.compare(_dbpass) != 0 )
		{
			//LOG_INFO("LOGIN CDescriptor::GetLogins pass.compare");
			throw MSG_FAIL_LOGINSERV_WRONG_PASSWORD;
		}
		//LOG_INFO("LOGIN CDescriptor::GetLogins Check Pass Done");
		// ###
#endif
