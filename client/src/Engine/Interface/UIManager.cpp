...
// ### For Verify strings static array
static size_t _iTotalStringsCount;
static size_t _iLastStringsIndex;
...
void CUIManager::LoadStringData( const CTFileName &fnString )
{
	...
	_iTotalStringsCount = iTotalCount;
	_iLastStringsIndex  = iLastIndex;

	// ### Debug
	// CPrintF("\nUIManager: m_aStringData.New(%d), TotalCount %d\n\n", iLastIndex + 1, iTotalCount);
...
inline ENGINE_API CTString _SFunc( int x )
{
	...
	// ### Verify:
	if (x < 0 || x > _iLastStringsIndex + 1) { // ### x < 0 SEGFAULT, x > _iLastStringsIndex + 1 -> static array out of bounds
		CTString _strString;
		_strString.PrintF("UIManager: FATAL ERROR! pUIManager->GetString(%d)", x);
		CPrintF("UIManager: iTotalCount  %d, iLastIndex %d\n", _iTotalStringsCount, _iLastStringsIndex);
		CPrintF("%s\n", _strString);
		return _strString;
	}

	// ### Debug
	//CPrintF("UIManager: pUIManager->GetString(%d) = ", x);
	//CPrintF("[%s]\n", pUIManager->GetString(x));
...
{
	//if (sam_iGfxAPI == GAT_OGL) { // ### opengl
		// ### Need implementation in Engine/Graphics/RenderTexture.cpp
		// ### now return
		// return;	
	//}
