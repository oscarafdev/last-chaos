...
void CUICharacterSelect::SetTextData( CUIListItem* pItem, int idx )
{
	...
	{
		CDrawPort* pDrawPort = CUIManager::getSingleton()->GetDrawPort(); // ### x64 rus
		ULONG ulWidth = pDrawPort->GetTextWidth2(strClass);               // ### x64 rus
...
void CUICharacterSelect::StartGame(/* int */size_t nSelectIdx)
{
	// 	if( PopUpMsgBoxExist() )
	// 		return
	...
	// ### Debug
	//CPrintF("CUICharacterSelect::StartGame() nSelectIdx[%d], ulStartIndex[%d]\n", nSelectIdx, ulStartIndex);
