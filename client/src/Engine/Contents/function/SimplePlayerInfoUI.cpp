...
void CSimplePlayerInfoUI::SetDungeonList(SLONG slExp)
{
	...
	// ### Debug
	// CPrintF("CSimplePlayerInfoUI::SetDungeonList m_pListDungeon->getListItemCount() = %d\n", nMax);
	CUIListItem* pItem = NULL;
	CUIText* pText = NULL;

	// ### Êòî êîðåéöêâ ó÷èë??? Çäåñü ïîñòîÿííî èäåò âûõîä çà ïðåäåëû ìàññèâà.
	// ### nMax = 18
	// ### Áåðåñòÿ ïðèèçâîëüíîå çíà÷åíèå èç ïàìÿòè ê ïðèâåðó -> UIManager: pUIManager->GetString(523013031) = [] - Ïðîñêàêèâàåò
	// ### Åñëè çíà÷åíèå îòðèöàòåëüíîå - ïðîèñõîäèò êðàõ -> UIManager: pUIManager->GetString(-449042348) = SEGFAULT
	// 
	// Êàê Ýòî âîîáùå ðàáîòàëî???
	// 
	// ### Óâåëè÷èâåì ìàññèâ è ôèêñèðóåì nMax
	int nStrIndex[/*14 ###*/ 18] = { 343/*1 º§ÇÇ½ºÆ® ½ÅÀü*/, 345/*3 ÇÁ·ÎÅ°¿Â ½ÅÀü*/, 1201/*9 ¸¶¸£°¡µÒ*/, 2368/*12 ¸Á°¢ÀÇ ½ÅÀü*/, 4946/*21 Å×¿À½ºÀÇ ¹«´ý*/,
		4947/*17, 18, 19, 20 ·ç½ºÆ® Åõ·ë*/, 5146/*37 Æ®¸®ºñ¾Æ °è°î*/, 4319/*29 ¿¡º¸´ÏÅº±¤*/, 4317/*30 ¹Ì½ºÆ¼Çù°î*/, 4395/*31 ÇÃ·Î¶óÀÓ µ¿±¼*/,
		4509/*34 ¿¹¹è´ç*/, 4510/*35 ¾ÏÈæÀÇ Á¦´Ü*/, 5065/*36 ¾ÆÄ­»ç¿ø*/, 5817/*41 ¶÷½½·Î ÁöÇÏ ¿¬±¸½Ç*/ 
		// ### Äîáàâëÿåì:
		, 343, 343, 343, 343// ###
	};
	
	/*
		UIManager : pUIManager->GetString(343)  = [Belfist Temple]
		UIManager : pUIManager->GetString(345)  = [Procyon Shrine]
		UIManager : pUIManager->GetString(1201) = [Maargadum]
		UIManager : pUIManager->GetString(2368) = [Temple of forgetfulness]
		UIManager : pUIManager->GetString(4946) = [Teo's Tomb]
		UIManager : pUIManager->GetString(4947) = [The Lust Trum.]
		UIManager : pUIManager->GetString(5146) = [Trivia Canyon]
		UIManager : pUIManager->GetString(4319) = [Ebony Mine]
		UIManager : pUIManager->GetString(4317) = [Misty canyon]
		UIManager : pUIManager->GetString(4395) = [The Floraim Cave]
		UIManager : pUIManager->GetString(4509) = [Cappella in Mondshine]
		UIManager : pUIManager->GetString(4510) = [Altar of Darkness in Mondshine]
		UIManager : pUIManager->GetString(5065) = [Akan Temple]
		UIManager : pUIManager->GetString(5817) = [Ramslo's Underground Laboratory]
	*/

	int nMaxWidth = 0;

	if (nMax > 18) {
		CPrintF("CSimplePlayerInfoUI::SetDungeonList nMax = [%d]->Array out of bounds!\n", nMax);
		nMax = 18;
	}
	...
	for (/* int */size_t i = 0; i < nMax; ++i)
	{
		...
		if (i == 0)
		{
			...
			if (slExp > 0)
				//strTemp.PrintF(CTString("%s(%d%%)"), _S(4959, "´øÀü ¸ó½ºÅÍ °æÇèÄ¡¸¦ Ãß°¡·Î È¹µæ"), slExp); / ###
				strTemp.PrintF(CTString("%s(%d%%)"), _S(4959, "Dai Li, Ge Jiao Pan, Ban Xiang Fu, Mian Ah Lung, Skirt Pad"), slExp);
			else
				//strTemp = _S(4959, "´øÀü ¸ó½ºÅÍ °æÇèÄ¡¸¦ Ãß°¡·Î È¹µæ"); // ###
				strTemp = _S(4959, "Dai Li, Ge Jiao Pan, Ban Xiang Fu, Mian Ah Lung, Skirt Pad");
		...
		{
			//pText->SetText(_S(4960, "´øÀü Å¸ÀÓ!")); // ###
			pText->SetText(_S(4960, "Dai Li Oulao!"));
			pText->setFontColor(0xA3A1A3FF);
		}
		else
		{
			//pText->SetText(_S(nStrIndex[i-2], "´øÀü ÀÌ¸§")); // ###
			pText->SetText(_S(nStrIndex[i - 2], "Take care of yourself"));
