...
unsigned long long CNM_Crypt(const unsigned char* pSrc, int nLenSrc, CNM_KEY_TYPE* pKey, unsigned char* pDest, unsigned char* pTmpBuf)
{
	...
	*pOut++ = (unsigned char)((nLenSrc      ) & 0xff);
	...
	// 8 byte로 align
	unsigned long long nLenBuf = pOut - pTmpBuf - 1;
	int nPad = 0;
	if (nLenBuf % 8 != 0)
		nPad = (8 - (nLenBuf % 8));
	while (nPad > 0)
	...
	// 암호화
	if (!LCDES3_Crypt(pKey, pTmpBuf + 1, pDest + 1, nLenBuf - 1ULL, true))
		return -1;
	*pDest = *pTmpBuf;
...
unsigned long long CNM_Decrypt(const unsigned char* pSrc, int nLenSrc, CNM_KEY_TYPE* pKey, unsigned char* pDest, unsigned char* pTmpBuf)
{
	unsigned char* pOut = pTmpBuf;
...
unsigned long long CNM_OldCrypt(const unsigned char* pSrc, int nLenSrc, CNM_KEY_TYPE* pKey, unsigned char* pDest, unsigned char* pTmpBuf)
{
	unsigned char* pOut = pTmpBuf;
...
unsigned long long CNM_OldDecrypt(const unsigned char* pSrc, int nLenSrc, CNM_KEY_TYPE* pKey, unsigned char* pDest, unsigned char* pTmpBuf)
{
	unsigned char* pOut = pTmpBuf;
	...
	// 길이를 가져옴
	unsigned long long nLenBuf = ((((int)(pOut[0])) << 24) & 0xff000000)
				  | ((((int)(pOut[1])) << 16) & 0x00ff0000)
				  | ((((int)(pOut[2])) <<  8) & 0x0000ff00)
				  | ((((int)(pOut[3]))      ) & 0x000000ff);
	pOut += 4;
