...
#ifdef UNICODE
ULONG_PTR WINAPI DisplayHTMLStr(HWND hwnd, const WCHAR *string)
#else
ULONG_PTR WINAPI DisplayHTMLStr(HWND hwnd, const char *string)
#endif
{	
...
#ifdef UNICODE
ULONG_PTR WINAPI DisplayHTMLPage(HWND hwnd, const WCHAR *webPageName)
#else
ULONG_PTR WINAPI DisplayHTMLPage(HWND hwnd, const char *webPageName)
#endif
{
	IWebBrowser2	*webBrowser2;
	VARIANT			myURL;
...
ULONG_PTR WINAPI EmbedBrowserObject(HWND hwnd)
{
	IOleObject					*browserObject;
	IWebBrowser2				*webBrowser2;
	RECT						rect;
	...
	if (!CoCreateInstance(&CLSID_WebBrowser, 0, CLSCTX_INPROC, &IID_IWebBrowser2, (void **)&webBrowser2))
	{
		...
		{
			SetWindowLongPtr(hwnd, GWLP_USERDATA, (ULONG_PTR)ptr);
